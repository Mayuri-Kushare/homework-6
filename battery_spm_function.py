...
def residual(t, SV, pars, ptr, flags):
    ...
    Q_rad = sigma*pars.emissivity*(pars.T_amb**4-(SV[ptr.T_an])**4)*pars.A_ext
    ...
    Q_rxn = -np.sum(sdot_k*energy_k)
    Q_ohm_el = pars.i_ext**2*pars.R_el_ca/pars.H_ca
    Q_ohm_io =  pars.i_ext**2*pars.R_io_ca
    Q_cond = Q_cond_ca/pars.H_ca
    Q_rad = sigma*pars.emissivity*(pars.T_amb**4-(SV[ptr.T_ca])**4)*pars.A_ext
